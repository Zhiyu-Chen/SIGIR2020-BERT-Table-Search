...
MODEL_CLASSES = {
    'bert': (BertConfig, BertForHybridTable, BertTokenizer)
}
...
class TableProcessor(object):
    ...
    """
    def __init__(self, data_dir='/root/data/table'):
        """
    ...
    @staticmethod
    def create_k_fold(data_dir='/root/data/table',k=5):
...
def load_and_cache_examples(args,tokenizer, split="train",scaler=None):
    ...
    str_features = [f.features for f in features]
    if 'train' in split and not scaler:
        scaler = preprocessing.MaxAbsScaler()
        str_features = scaler.fit_transform(str_features)
    else:
        str_features = scaler.transform(str_features)

    # Convert to Tensors and build dataset
    all_input_ids = torch.tensor([f.input_ids for f in features], dtype=torch.long)
    all_features = torch.tensor([each for each in str_features],dtype=torch.float)
    ...
    return dataset, examples,scaler # also return examples for post-processing


def train(args, train_dataset, model, tokenizer,scaler):
    if args.local_rank in [-1, 0]:
        if args.k_fold != 0:
    ...
    for _ in train_iterator:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        if args.k_fold == 0:
                            ...
                            results = evaluate(args, model, tokenizer,
                                               prefix='test ' + str(global_step) + ' :epoch ' + str(epoch_num) + ' fold ' + str(args.fold),split = str(args.fold) + '_test',scaler=scaler)
        ...
        if args.k_fold != 0:
            results = evaluate(args, model, tokenizer, prefix='epoch '+ str(epoch_num),split=str(args.fold) + '_test',scaler=scaler)
        else:
            results = evaluate(args, model, tokenizer, prefix='epoch ' + str(epoch_num)+ ' fold ' + str(args.fold), split= 'test',scaler=scaler)
        for key, value in results.items():
            tb_writer.add_scalar('test_eval_epoch_{}'.format(key), value, epoch_num)
        if args.k_fold != 0:
            train_results = evaluate(args, model, tokenizer, prefix='epoch '+ str(epoch_num)+ ' fold ' + str(args.fold), split=str(args.fold) + '_train',scaler=scaler)
        else:
            train_results = evaluate(args, model, tokenizer, prefix='epoch ' + str(epoch_num),split='train',scaler=scaler)
        for key, value in train_results.items():
...
def evaluate(args, model, tokenizer, prefix="",split="test",scaler=None):
    eval_output_dir = args.output_dir
    eval_dataset, eval_examples,scaler = load_and_cache_examples(args, tokenizer, split,scaler)
...
def main():
    ...
    parser.add_argument("--content", default=None, type=str, required=False,
                        help="how to concatenate the table content.")
    ...
    if args.do_train and args.k_fold == 0:
        ...
        model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path),
                                            ...
                                            load_averaged_checkpoint=args.load_averaged_checkpoint,feature_len=args.feature_len)
        ...
        model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path),
                                            ...
                                            ignore_logits_layer=args.ignore_logits_layer,
                                            ignore_sequence_summary_layer=args.ignore_sequence_summary_layer,
                                            load_averaged_checkpoint=args.load_averaged_checkpoint,feature_len=args.feature_len,trans_len = args.trans_len)
        ...
        train_dataset, _,scaler = load_and_cache_examples(args, tokenizer, split=str(args.fold) + "_train")
        global_step, tr_loss = train(args, train_dataset, model, tokenizer,scaler)
        logger.info(" global_step = %s, average loss = %s, fold = %s", global_step, tr_loss,fold)
        ...
        #evaluate
        results = evaluate(args, model, tokenizer, prefix='fold ' + str(fold), split=str(args.fold) + '_test',scaler=scaler)
        torch.save(results, os.path.join(args.output_dir, 'cache_rs_fold_{0}.bin'.format(str(fold))))
